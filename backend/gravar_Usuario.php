...
$senha_confirmar = mysqli_real_escape_string($conexao, $_POST['senha_confirmar']);
$status = "Inativo";
...
if ($_POST['cod']) {
  ...
  if (!empty($nome) && !empty($email) && !empty($senha) && !empty($senha_confirmar)) {
    ...
    if ($senha_confirmar == $senha) {
      ...
      $sql2 = mysqli_query($conexao, "UPDATE usuarios 
                                    SET nome='$nome', email='$email', senha='$encrypt_pass', tipo='$tipo', status='$status'
  ...
  if (!empty($nome) && !empty($email) && !empty($senha) && !empty($senha_confirmar) && !empty($tipo)) {
    $sqlNome = mysqli_query($conexao, "SELECT nome FROM usuarios WHERE nome = '{$nome}'");
